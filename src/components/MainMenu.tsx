...
import { Trophy, Users, Play, PlusCircle } from 'lucide-react';

interface MainMenuProps {
  onJoinGame: (playerName: string, isAI: boolean, roomId?: string) => void;
}

export const MainMenu: React.FC<MainMenuProps> = ({ onJoinGame }) => {
  const [playerName, setPlayerName] = useState('Player 1');
  const [view, setView] = useState<'main' | 'lobby' | 'leaderboard'>('main');
        ...
        {view === 'main' && (
          ...
          >
            <input
              ...
              className="w-full px-6 py-4 rounded-xl text-2xl font-bold bg-white/10 border-4 border-white/20 text-white placeholder-white/50 text-center focus:outline-none focus:border-yellow-400 transition-colors"
              placeholder="Enter your name"
            />
            
            <button
              onClick={() => onJoinGame(playerName, true)}
        ...
        {view === 'lobby' && (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              <button
                onClick={() => onJoinGame(playerName, false, 'CREATE')}
